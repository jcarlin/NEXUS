...
6. **Finish the loop: implement → test → fix → verify → update ROADMAP.** After completing a plan, run `pytest tests/ -v`. If tests fail, fix the issues and re-run until all tests pass. Only then update `ROADMAP.md` to reflect what was completed (check off items, update status, adjust test counts). Work is not done until the roadmap is current.
