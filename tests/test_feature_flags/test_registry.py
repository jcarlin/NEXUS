...
class TestFlagRegistry:
    ...
    def test_expected_flag_count(self):
        """Registry should have one entry per flag Settings field (excluding aliases)."""
        # enable_visual is an alias for enable_visual_embeddings, not a standalone flag
        # defer_ner_to_queue is a flag that doesn't use the enable_ prefix
        flag_fields = {k for k in Settings.model_fields if k.startswith("enable_") and k != "enable_visual"}
        flag_fields |= {k for k in FLAG_REGISTRY if not k.startswith("enable_")}
